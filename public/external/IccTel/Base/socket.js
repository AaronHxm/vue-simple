...
wsk = {
    ...
    onOpen: function (evt) {
        ...
        wsk.isConnect = true;
        wsk.checkIn();
        this.heartbeat_timer = setInterval(function () { wsk.keepalive(websocket) }, 50000);
    },
    ...
    onMessage: function (evt) {
        ...
        if (evt.data.length > 4) {
            ...
            if (json.state == 0) //ok
            {
                ...
                else if (json.cmdsn == "119") {
                    SetSeatNo(json.para);
                }
                else if (json.cmdsn == "120") {
                    SetBusyNode(json.para);
                }
                else if (json.cmdsn == "121") {
                    SetIvr(json.para);
                }
                else if (json.cmdsn == "122") {
                    SetExt(json.para);
                }
                ...
                else if (json.cmdsn == "115" || json.cmdsn == "116") {
                    keepRegainOper("whfhold1", "whfhold2");
                }
            }
            ...
            {
                IccCallin(json.caller, json.cmdsn);
                this.outms('有新来电');
            }
            if (json.state == 2) //station callout
            {
                ...
                IccCallout(json.para, json.cmdsn);
                this.outms(callercalledinfo);
            }
            if (json.state == 3) {
                //通话开始(无论是呼入还是呼出)
            ...
            if (json.cmdsn == "999") {
                document.getElementById("showstate").innerText = evt.data;
            }
        }
    },
    ...
    TransSeatnoNew: function (autherseatno) {
        //转分机,可以转没有签入的坐席工号分机
        str1 = '{"cmdsn":"1090","seatno":"' + wsk.txtseatno + '","caller":"' + wsk.txtseatno + '","para":"' + autherseatno + '","cmd":"16"}';
        websocket.send(str1);
    },
    GotoGroup: function (groupid) {
        str1 = '{"cmdsn":"1091","seatno":"' + wsk.txtseatno + '","caller":"","para":"' + groupid + '","cmd":"9"}';
        websocket.send(str1);
    },
    ...
    //2018-06-26 增加的三方会议功能
    StopCall: function (externtion) {
        str1 = '{"cmdsn":"114","seatno":"' + wsk.txtseatno + '","caller":"","para":"' + externtion + '","cmd":"41"}';
        websocket.send(str1);
    },
    StopCallTwo: function () {
        str1 = '{"cmdsn":"115","seatno":"' + wsk.txtseatno + '","caller":"","para":"","cmd":"42"}';
        websocket.send(str1);
    },
    StopCallThree: function () {
        str1 = '{"cmdsn":"116","seatno":"' + wsk.txtseatno + '","caller":"","para":"","cmd":"43"}';
        websocket.send(str1);
    },
    InsideCall: function (autherTel) {
        str1 = '{"cmdsn":"117","seatno":"' + wsk.txtseatno + '","caller":"' + wsk.txtseatno + '","para":"' + autherTel + '","cmd":"44"}';
        websocket.send(str1);
    },
    InsideMeet: function () {
        str1 = '{"cmdsn":"118","seatno":"' + wsk.txtseatno + '","caller":"","para":"","cmd":"45"}';
        websocket.send(str1);
    },
    SendSocketStr: function (wsstr) {
        if (websocket != undefined && websocket != null) {
            websocket.send(wsstr);
            document.getElementById("showstate").innerText = "命令已发送";
        }
        else {
            document.getElementById("showstate").innerText = "请初始化Socket,签入";
        }
    },
    //2018-06-26三方会议结束
    //2019-09-17获取在线坐席,技能组
    GetSeatInfo: function () {
        str1 = '{"cmdsn":"119","seatno":"' + wsk.txtseatno + '","caller":"","para":"1","cmd":"63"}';
        websocket.send(str1);
    },
    GetBusyNode: function () {
        str1 = '{"cmdsn":"120","seatno":"' + wsk.txtseatno + '","caller":"","para":"3","cmd":"63"}';
        websocket.send(str1);
    },
    GetIvrInfo: function () {
        str1 = '{"cmdsn":"121","seatno":"' + wsk.txtseatno + '","caller":"","para":"2","cmd":"63"}';
        websocket.send(str1);
    },
    GetExtInfo: function () {
        str1 = '{"cmdsn":"122","seatno":"' + wsk.txtseatno + '","caller":"","para":"4","cmd":"63"}';
        websocket.send(str1);
    },
    //后台班长签入
    //para 传入的是工号
    AdminCheckIn: function (para) {
        str1 = '{"cmdsn":"122","seatno":"' + wsk.txtseatno + '","caller":"","para":"' + para + '","cmd":"21"}';
        websocket.send(str1);
    }
};
