 ...
 * @version 1.2.10 (2018-06-26) Reason:增加了对应的三方会议的并线功能
 * @version 2.1.10 (2019-09-18) Reason:升级了三方会议的功能,升级了三方通话,升级了空闲功能,升级了转接功能
 * @version 2.2.10 (2019-09-20) Reason:替换了对应的按钮文字，已经替换了尾部的 工号，状态，通时，排队
 *******************************************************************************/

(function (window, undefined) {
  //工号,分机,分机密码,icc服务地址
  var seatNo = '8008', seatExt = '8008', seatPassWord = '123456', iccSerVicePath = 'ws://icc.maixintech.com:55061';

  var _VERSION = '1.2.10 (2018-06-26)',/*版本号*/
  ...
  //属性
  var _ITEMS = ('checkIn,checkOut,txtTel,callTel,dropTel,threeCall,threeMeet,dialBack,threeTalk,killTalk,transIvr,transSeatno,transExt,freeCall,keepCall,keepQuiet').split(',');
  P.options = {
    seatNo: seatNo,
    seatExt: seatExt,
    ...
    cssPath: P.basePath + 'Base/default/',
    langType: 'zh_CN',
    zIndex: 9999,
    items: [
      'checkIn', 'checkOut', 'txtTel', 'callTel', 'dropTel', 'threeCall', 'threeMeet', 'dialBack',
  ...
  function _create(expr, options) {
    ...
    P.options.ThreeTel = _undef(options.ThreeTel, P.options.ThreeTel);
    P.options.langType = _undef(options.langType, P.options.langType);
    P.options.items = _undef(options.items, P.options.items)
    P.options.cssPath = _undef(options.cssPath, P.options.cssPath);//Css
    ...
    //加载样式
    // _loadStyle(P.options.cssPath + 'Tel.css');
    ...
    _loadScript(P.options.themesPath + 'base64.min.js');
  ...
  function parentDiv(knode) {
    var self = knode;
    var width = knode.style.width || 1204 + "px";
    var height = knode.style.height || 47 + "px";
    container = document.createElement("div");
    container.className = "icctel-container";
    container.setAttribute("style", "width:" + width + ";height:" + height + ";");
    for (var i = 0, j = knode.style.length; i < j; i++) {
      if (knode.style[i] != "visibility") {
        container.style.cssText += knode.style[i] + ":" + getComputedStyle(knode, null).getPropertyValue(knode.style[i]) + ";";
      }
    }
  ...
  //LinkButton Input
  var ICCLINK = ',checkIn,checkOut,callTel,dropTel,threeCall,threeMeet,dialBack,threeTalk,killTalk,transIvr,transSeatno,transExt,freeCall,keepCall,keepRegain,keepQuiet,';
  var ICCINPUT = ',txtTel,';
  //ID组合
  var IccId = new Array();
  ...
  //点击事件 IccOnclick["freeCall"] = "freeCall(1)";
  ...
  IccOnclick["threeCall"] = "threeParts()"; IccOnclick["threeMeet"] = "threeMeet()"; IccOnclick["dialBack"] = "dialBack()"; IccOnclick["threeTalk"] = "threeTalk()"; IccOnclick["killTalk"] = "killTalk()";
  IccOnclick["transIvr"] = "transIvr()"; IccOnclick["transSeatno"] = "transSeatno()"; IccOnclick["transExt"] = "transExt()"; IccOnclick["keepCall"] = "keepCall(1)"; IccOnclick["keepRegain"] = "keepRegain(0)";
  IccOnclick["keepQuiet"] = "keepQuiet(2)";
  //鼠标移入事件
  var IccOnMouseEnter = new Array();
  IccOnMouseEnter["freeCall"] = "freeMouseOver(this)";
  var IccOnMouseLeave = new Array();
  //IccOnMouseLeave["freeCall"] = "freeMouseLeave()";

  //2019-09-17 增加如下功能
  var IccBusyArr = [["0", "空闲"]];//繁忙小类设置
  var IccSeatArr = new Array();//坐席数组
  var IccIvrArr = new Array();//技能组数组
  var IccExtArr = new Array();//分机组数组

  P.IccOnclick = IccOnclick;
  P.IccOnMouseEnter = IccOnMouseEnter;
  P.IccBusyArr = IccBusyArr;
  P.IccSeatArr = IccSeatArr;
  P.IccIvrArr = IccIvrArr;
  P.IccExtArr = IccExtArr;

  //Icon图标
  var IccIcon = new Array();
  IccIcon["checkIn"] = "Icc-l-btn-icon icon-cc-denglu color-shenlan"; IccIcon["checkOut"] = "Icc-l-btn-icon icon-cc-dengchu color-shenlan"; IccIcon["callTel"] = "Icc-l-btn-icon icon-caller color-shenlan";
  IccIcon["dropTel"] = "Icc-l-btn-icon icon-cc-guaji color-shenlan"; IccIcon["threeCall"] = "Icc-l-btn-icon icon-cc-sanfang color-shenlan"; IccIcon["threeMeet"] = "Icc-l-btn-icon icon-cc-sanfang color-shenlan";
  IccIcon["dialBack"] = "Icc-l-btn-icon icon-cc-pingjia color-shenlan"; IccIcon["threeTalk"] = "Icc-l-btn-icon icon-answer color-shenlan"; IccIcon["killTalk"] = "Icc-l-btn-icon icon-hangup color-shenlan";
  IccIcon["transIvr"] = "Icc-l-btn-icon icon-cc-bohaopan color-shenlan"; IccIcon["transSeatno"] = "Icc-l-btn-icon icon-link color-shenlan"; IccIcon["transExt"] = "Icc-l-btn-icon icon-link color-shenlan";
  IccIcon["freeCall"] = "Icc-l-btn-icon icon-tousuguanli color-shenlan"; IccIcon["keepCall"] = "Icc-l-btn-icon icon-ringpause color-shenlan";
  IccIcon["keepRegain"] = "Icc-l-btn-icon icon-ring color-shenlan"; IccIcon["keepQuiet"] = "Icc-l-btn-icon icon-ringpause color-shenlan";
  //增加text文件
  ...
  IccButtonText["threeCall"] = "_language[\"@langType@\"].core.threeCall"; IccButtonText["threeMeet"] = "_language[\"@langType@\"].core.threeMeet";
  IccButtonText["dialBack"] = "_language[\"@langType@\"].core.dialBack";
  IccButtonText["threeTalk"] = "_language[\"@langType@\"].core.threeTalk"; IccButtonText["killTalk"] = "_language[\"@langType@\"].core.killTalk";
  IccButtonText["transIvr"] = "_language[\"@langType@\"].core.transIvr"; IccButtonText["transSeatno"] = "_language[\"@langType@\"].core.transSeatno";
  IccButtonText["transExt"] = "_language[\"@langType@\"].core.transExt"; IccButtonText["freeCall"] = "_language[\"@langType@\"].core.freeCall";
  ...
  function createLink(_this, arr) {
    ...
    //if (_undef(IccOnMouseEnter[_this], "") != "") {
    //    node.setAttribute("onmouseenter", IccOnMouseEnter[_this]);
    //}
    if (_this == "keepRegain") {
  ...
  //    node2_1.appendChild(document.createTextNode(eval("_language[\"" + P.options.langType + "\"].core.callLongTime") + "："));//
  //    node2.appendChild(node2_1);
  //    var node2_1_1 = document.createElement("span");
  //    node2_1_1.id = "IcctimerSpan";
  //    node2_1_1.appendChild(document.createTextNode("00:00:00"));
  ...
  //    node3_2.appendChild(document.createTextNode(eval("_language[\"" + P.options.langType + "\"].core.seatNo") + "："));
  //    node3.appendChild(node3_2);
  //    var node3_2_1 = document.createElement("span");
  //    node3_2_1.style = "color: #07a4a9";
  //    node3_2_1.appendChild(document.createTextNode(P.options.seatExt));
  ...
  function createRightInfo() {
    ...
    span_1.appendChild(document.createTextNode(eval("_language[\"" + P.options.langType + "\"].core.workNumber") + "："));
    node_1.appendChild(span_1);
    var span_1_1 = document.createElement("span");
    span_1_1.appendChild(document.createTextNode(P.options.seatNo));
    node_1.appendChild(span_1_1);
    //通时
    var span_2 = document.createElement("span");
    span_2.appendChild(document.createTextNode(eval("_language[\"" + P.options.langType + "\"].core.callLongTime") + "："));
    node_1.appendChild(span_2);
    var span_2_1 = document.createElement("span");
    span_2_1.id = "IcctimerSpan";
    span_2_1.appendChild(document.createTextNode("00:00"));
    node_1.appendChild(span_2_1);
    ...
    span_3.appendChild(document.createTextNode(eval("_language[\"" + P.options.langType + "\"].core.seatExtSate") + "："));
    node_2.appendChild(span_3);
    var span_3_1 = document.createElement("span");
    span_3_1.id = "Iccstate";
    span_3_1.appendChild(document.createTextNode("离线"))
    node_2.appendChild(span_3_1);
    //排队
    var span_4 = document.createElement("span");
    span_4.appendChild(document.createTextNode(eval("_language[\"" + P.options.langType + "\"].core.lineUp") + "："));
    node_2.appendChild(span_4);
    var span_4_1 = document.createElement("span");
    span_4_1.id = "IcclineUp";
    span_4_1.appendChild(document.createTextNode("0"));
    node_2.appendChild(span_4_1);
  }
